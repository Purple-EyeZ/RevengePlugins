...
patches.push(
    after('generate', RowManager.prototype, ([data], row) => {
        ...
        const jumbo = content.every(
            c => (c.type === 'link' && c.target.match(emojiRegex)) || (c.type === 'text' && c.content === ' '),
        )
        ...
        for (let i = 0; i < content.length; i++) {
            ...
            const isAnimated = el.target.includes('animated=true')

            const coreUrl = `https://cdn.discordapp.com/emojis/${match[1]}.${isAnimated ? 'gif' : 'webp'}`

            const url = `${coreUrl}?size=128`
